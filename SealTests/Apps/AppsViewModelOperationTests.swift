...
struct AppsViewModelOperationTests {
    ...
    @Test
    func postInstallCleanupPreservesAnotherLeasedAppsWorkspaceAndSignedIPA() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        ...
        #expect(try await fixture.fileStore.exists(relativePath: installedPath) == false)
        let active = try #require(await fixture.appStore.fetchAll().first { $0.id == activeID })
        #expect(active.signedIPARelativePath == activePath)
    }

    @Test
    func batchPostInstallCleanupSkipsLeasedAppsSignedIPAAndMetadata() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        let activeID = UUID()
        let idleID = UUID()
        ...
        #expect(try await fixture.fileStore.exists(relativePath: activePath))
        #expect(try await fixture.fileStore.exists(relativePath: idlePath) == false)
        let records = await fixture.appStore.fetchAll()
        #expect(records.first { $0.id == activeID }?.signedIPARelativePath == activePath)
        #expect(records.first { $0.id == idleID }?.signedIPARelativePath == nil)
    }
