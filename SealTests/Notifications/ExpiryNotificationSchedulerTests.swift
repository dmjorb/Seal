...
struct ExpiryNotificationSchedulerTests {
    ...
    @Test
    func statusSnapshotReportsAuthorizationPendingCountAndNextDate() async throws {
        let center = ControllableNotificationCenter()
        await center.allowAuthorization(granted: true)
        let scheduler = ExpiryNotificationScheduler(center: center)
        let app = makeApp()

        _ = try await scheduler.setEnabled(true, apps: [app])
        let snapshot = await scheduler.statusSnapshot()

        #expect(snapshot.authorization == .authorized)
        #expect(snapshot.pendingCount == 1)
        #expect(snapshot.nextFireDate != nil)
    }
