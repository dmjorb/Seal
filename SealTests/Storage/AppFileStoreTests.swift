...
struct AppFileStoreTests {
    ...
    @Test
    func removingTemporaryExportKeepsFormalSignedIPA() async throws {
        ...
        let appID = UUID()
        let signedPath = try await store.storeSignedIPA(
            sourceURL: fixture.source,
            appID: appID
        )
        let exportURL = try await store.prepareSignedIPAExport(
            relativePath: signedPath,
            fileName: "Export.ipa"
        )

        try await store.removeSignedIPAExport(at: exportURL)
    ...
    @Test
    func storedSignedIPAUsesVersionedPathAndDetectsSHA256Mismatch() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        let store = AppFileStore(
            documentsDirectory: fixture.documents,
        ...
        #expect(signedPath.contains("Signed-"))
        let metadata = try await store.verifySignedIPA(relativePath: signedPath)
        #expect(metadata.relativePath == signedPath)
        #expect(metadata.byteCount == 3)
        #expect(metadata.sha256.count == 64)

        let storedURL = try await store.fileURL(relativePath: signedPath)
        try Data("changed".utf8).write(to: storedURL, options: .atomic)
        do {
            _ = try await store.verifySignedIPA(relativePath: signedPath)
            Issue.record("Expected SHA-256 verification to fail")
    ...
    @Test
    func signedIPARemovalRecoveryRestoresArtifactWhileRecordStillReferencesIt() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        let store = AppFileStore(
            documentsDirectory: fixture.documents,
        ...
        let record = AppRecord(
            id: appID,
            originalBundleIdentifier: "com.example.rollback",
            mappedBundleIdentifier: "com.example.rollback.seal",
            name: "Rollback",
            version: "1.0",
            buildNumber: "1",
            size: 3,
        ...
        _ = try await store.prepareSignedIPARemoval(appID: appID)
        #expect(try await store.exists(relativePath: signedPath) == false)

        try await store.recoverSignedIPARemovals(appRecords: [record])

        #expect(try await store.exists(relativePath: signedPath))
        #expect(try await store.verifySignedIPA(relativePath: signedPath).sha256.count == 64)
    }

    @Test
    func signedIPARemovalRecoveryFinalizesArtifactAfterRecordClearsReference() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        let store = AppFileStore(
            documentsDirectory: fixture.documents,
        ...
        var record = AppRecord(
            id: appID,
            originalBundleIdentifier: "com.example.finalize",
            mappedBundleIdentifier: "com.example.finalize.seal",
            name: "Finalize",
            version: "1.0",
            buildNumber: "1",
            size: 3,
        ...
        _ = try await store.prepareSignedIPARemoval(appID: appID)
        record.signedIPARelativePath = nil
        try await store.recoverSignedIPARemovals(appRecords: [record])
    ...
    @Test
    func signedArtifactRecoveryRemovesUnreferencedVersionsAndKeepsReferencedFile() async throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        let store = AppFileStore(
            documentsDirectory: fixture.documents,
        ...
        let orphanID = UUID()
        let referencedPath = try await store.storeSignedIPA(
            sourceURL: fixture.source,
            appID: referencedID
        )
        let orphanPath = try await store.storeSignedIPA(
            sourceURL: fixture.source,
            appID: orphanID
        )
        let record = AppRecord(
            id: referencedID,
            originalBundleIdentifier: "com.example.referenced",
            mappedBundleIdentifier: "com.example.referenced.seal",
            name: "Referenced",
            version: "1.0",
            buildNumber: "1",
            size: 3,
            state: .signed,
            ipaRelativePath: "Apps/\(referencedID.uuidString)/Original.ipa",
            signedIPARelativePath: referencedPath,
            importedAt: Date()
        )

        try await store.reconcileSignedIPAArtifacts(appRecords: [record])

        #expect(try await store.exists(relativePath: referencedPath))
        #expect(try await store.exists(relativePath: orphanPath) == false)
    }
