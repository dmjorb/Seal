...
Sources/
  ...
  Install/Jit/Provision          Remote RSD service facades
RustBridge/
...
Seal's signing, IPA installation, installation verification, and renewal flow do
not require personalized DDI mounting. The pinned `idevice` revision does not
expose a stable personalized-mount RSD API, so `Mounter.startAutoMounter` reports
that the feature is unavailable instead of retrying forever or claiming success.
