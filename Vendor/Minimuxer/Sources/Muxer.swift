...
public class Muxer {
        ...
        switch messageType {
            ...
            case "Listen":
                if let deviceIP = currentDeviceIP,
                   let payload = try? buildPayload(deviceIP: deviceIP, event: currentEvent) {
                    let pkt = RawPacket(plist: payload, version: 1, message: 8, tag: 0)
                    sendData(pkt.data, to: fd)
                }
    ...
    private static func sendData(_ data: Data, to fd: Int32) {
